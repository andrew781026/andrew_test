...
public class List_ResultSetToExcel {
	...
	private ArrayList<ResultSet> resultSet_List;
	private int Row_Flag = 0;

	public List_ResultSetToExcel(ResultSet resultSet, FormatType[] formatTypes,
			String sheetName) {
	...
	public List_ResultSetToExcel(ResultSet resultSet, String sheetName) {
	...
	private FormatType getFormatType(int Type) {
	...
	public void generate(File file, ArrayList<ResultSet> resultSet_List)
			throws Exception {
		for (ResultSet resultSet : resultSet_List) {
			this.resultSet = resultSet;
			this.generate(new FileOutputStream(file));
		}
	...
	public void generate(OutputStream outputStream) throws Exception {
		...
		try {
			if (Row_Flag < 65535) {
				...
				if (formatTypes != null
						&& formatTypes.length != resultSetMetaData
								.getColumnCount()) {
				...
				//print out				
				System.out.println("Row_Flag = " + Row_Flag);
				
				int currentRow = Row_Flag;
				...
				if (Row_Flag == 0) {
					...
					for (int i = 0; i < numCols; i++) {
						...
						if (isAutoDecideFormatTypes) {
							...
							if (getFormatType(_class) == FormatType.TEXT) {
								int _Type = resultSetMetaData
										.getColumnType(i + 1);
								formatTypes[i] = getFormatType(_Type);
							}
			...
			/* if(resultSet_List != null) */workbook.write(outputStream);

		} finally {
			/* if(resultSet_List != null) */outputStream.close();
		}
